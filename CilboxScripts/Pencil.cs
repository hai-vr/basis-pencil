...
namespace Hai.Basis.CilboxPencil
{
    ...
    public class Pencil : MonoBehaviour
    {
        ...
        private const int RaycastMask = ~((1 << 2) | (1 << 3) | (1 << 5) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9) | (1 << 10) | (1 << 11));
        ...
        private void WhenEnable()
        {
            ...
            // instantiateMe_Renderer.material = mainLineRenderer.material;
        ...
        private void Update()
        {
            if (_isTriggerMode) return;

            if (_isPickedUp)
            {
                var raycastPos = tip.position - tip.forward * 0.3f;
                if (Physics.Raycast(raycastPos, tip.forward, out var hitInfo, 0.3f + 0.01f, RaycastMask))
                {
                    modelMover.position = hitInfo.point;
                    _lastPressingOnColliderRotation = Quaternion.LookRotation(-hitInfo.normal, tip.up);
                    _lastPressingOnColliderPosition = hitInfo.point + hitInfo.normal * 0.0001f;
...
#region Networking
    ...
    private void Submit(ushort[] recipientsNullable)
    {
        ...
        internal void PrepareSeilerInterpolation(Vector3 pos0, Vector3 pos1, Vector3 direction0, Vector3 direction1, out Vector3 b0, out Vector3 b3, out Vector3 s1, out Vector3 s2)
        {
            b0 = pos0;
            b3 = pos1;
        ...
        internal Vector3 SeilerInterpolate(Vector3 b0, Vector3 b3, Vector3 s1, Vector3 s2, float t)
        {
